...
- **無人值守模式**：Windows 工作排程器每天 8:00 自動執行，完成後在 Gmail 建立一封**草稿**（不會寄出），HTML 報表以附件夾帶，收件人 peggy.wu@rehfeldt.org、副本 supportlf@rehfeldt.org。台灣的國定假日（依行政院人事行政總處公告，不含純週末）不觸發
...
- 國定假日判斷抓的是 [行政院人事行政總處公告的官方日曆資料](https://github.com/ruyut/TaiwanCalendar)（線上抓取，非離線套件），抓不到資料（網路問題等）時保守當作「不是假日」照常執行，不會因為抓不到假日資料而漏掉真正的工作日；純週末不算「國定假日」，還是會照常執行。
