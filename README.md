...
5. 依截止投標時間排序，輸出成 `output/tenders_<時間戳記>.html`
6. 無人值守模式：呼叫 Gmail API 建立草稿信，HTML 檔案當附件
...
- **觸發程序**：需要兩個，缺一個都可能整天不會跑：
  - **登入時**，使用者選帳號本身——涵蓋真的登出後重新登入的情況
  - **工作站解除鎖定時**（GUI 上「開始工作」選單裡選 "On workstation unlock"）——涵蓋只是螢幕鎖定/電腦睡眠、解鎖恢復工作的情況，**這是日常最常發生的模式**，只設「登入時」幾乎不會觸發
- **動作**：
...
- 假日判斷抓的是 [行政院人事行政總處公告的官方日曆資料](https://github.com/ruyut/TaiwanCalendar)（線上抓取，非離線套件，已正確處理補假、補班日），抓不到資料（網路問題等）時保守當作「不是假日」照常執行，不會因為抓不到假日資料而漏掉真正的工作日。
